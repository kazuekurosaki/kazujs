...
!function KotoThemeScript() {
    // Variable declarations - storing references to common functions/objects
    var M = window,
        aQ = document,
        a = setTimeout,
        b9 = clearTimeout,
    ...
    cB = function(cK) {
        ...
        function cJ(cU, cS, i) {
            ...
            return I(cT, "btn btn-sm rounded-pill jt-icon-center"), cT[be] = i || cU, cT[bk + a2]("data-page", cU), cU == cS ? I(cT, "jt-btn-primary") : (I(cT, "jt-btn-light hover-btn-primary"), cT[cc + a4](G, function(cW) {
                var cV;
                cW[ca](), 1 == (cO = cT[bB + a2]("data-page")) ? (cV = cI ? aG + "/search" + cN + "?max-results=" + cH + "&page=" + cO : aG, M[bf][X] = cV) : (cV = (cO - 1) * cH, Defer.js(aG + "/feeds/posts/summary/" + cQ + "?start-index=" + cV + "&alt=json&callback=KazuJs." + e + "_date&max-results=1"))
            })), cR[cy](cT), cR
        }
        cK[bB + a2]("data-pagination") != x ? (cH = cK[bB + a2]("data-posts"), cI = cK[bB + a2]("data-label"), cQ = (cI = bP(cI)) ? "-/" + cI + "/" : "", cN = cI ? "/label/" + cI : "", Defer.js(aG + "/feeds/posts/summary/" + cQ + "?alt=json&callback=KazuJs." + e + "_" + bs + "&max-results=1"), cM = co("max-results", aO), cP = co("page", aO), cL = cM || cH, cO = cP || 1, KazuJs[e + "_" + bs] = function(cU) {
            var cS = cU.feed,
    ...
    KazuJs[ct] = function(cM) {
        ...
        Defer.js(aG + "/feeds/posts/summary/" + cL + "alt=json&callback=KazuJs." + k + "_" + bs + "_" + cH + "&max-results=" + cR), KazuJs[k + "_" + bs + "_" + cH] = function(cX) {
    ...
    // Update page titles for pagination links
    var y = function(cK) {
    ...
    // Close on outside click function
    function z(cI, i) {
        function cJ(cK) {
            cI[V](cK[L]) || (i(), cH())
        }
    ...
    function c(cQ) {
        if (cQ && (aQ[aM + a4](bL, c), aQ[aM + a4](Y, c), aQ[aM + a4](bV, c)), Defer.dom(".custom-posts", 1, null, KazuJs[ct], null, {
                rootMargin: "200%"
            }), 0 < H[cx]) {
    ...
    // Page title update for pagination
    ae && (aQ[aT] = aQ[aT][D](br, br + b6 + " " + ae + br)),

    // Initialize components
    Defer.dom(".lazyload", 1, "loaded", E), Defer.dom("#post-pager", 1, null, y, null, {
        rootMargin: "200%"
